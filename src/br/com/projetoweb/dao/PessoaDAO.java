...
public class PessoaDAO {
	...
	public static Pessoa getPessoaById(int id) {
		...
		try {
			...
			while (rsPessoa.next()) {
				...
				pessoaRetorno.setCpf(rsPessoa.getString("cpf"));
	...
	public static int cadastraPessoa(Pessoa pessoaSubmit) {
		String query = "INSERT INTO pessoa(nome, dt_nascimento, cpf, sexo) values (?, ?, ?, ?)";
		Connection cnn = ConnectionFactory.getConnection();
		...
		try {
			...
			pStmt.setString(3, pessoaSubmit.getCpf());
	...
	public static int updatePessoa(Pessoa pessoaSubmit) {
		String query = "UPDATE pessoa SET nome = ?, dt_nascimento = ?, cpf = ? WHERE id= ?";
		Connection cnn = ConnectionFactory.getConnection();
		int linhasAfetadas = 0;
		...
		try {
			...
			pStmt.setString(2, pessoaSubmit.getDtNascimento());
			pStmt.setString(3, pessoaSubmit.getCpf());
			pStmt.setLong(4, pessoaSubmit.getId());
			linhasAfetadas = pStmt.executeUpdate();
			cnn.close();
		} catch (SQLException e) {
			e.printStackTrace();
